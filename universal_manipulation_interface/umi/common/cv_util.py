...
def custom_plot(frame, results, model, object):
    annotated_frame = frame.copy()
    if object == "cup":
        obstacle = "bottle"
    if object == "bottle":
        obstacle = "cup" 
    for box, cls in zip(results[0].boxes.data, results[0].boxes.cls):
        x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])
        cls = int(cls)
        class_name = model.names[cls]
        if class_name in obstacle or class_name in object:
            color = (0, 255, 0) if class_name == object else (0, 0, 255) # object green, obstacle red
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 10)
        else :
            continue
    return annotated_frame

def custom_seg(frame, results, model, object):
    annotated_frame = frame.copy()
    ...
    for mask, cls in zip(results[0].masks.data, results[0].boxes.cls):
        ...
        if class_name in obstacle or class_name in object:
            ...
            annotated_frame[binary_mask] = (annotated_frame[binary_mask] * 0.8 + np.array(color) * 0.20).astype(np.uint8)
    return annotated_frame