...
class RTDEInterpolationController(mp.Process):
    ...
    def pause(self, wait=True):
        message = {
            'cmd': Command.PAUSE.value
        }
        self.input_queue.put(message)

    def unpause(self, wait=True):
        message = {
            'cmd': Command.UNPAUSE.value
        }
        self.input_queue.put(message)
        ...
        try:
            ...
            while keep_running:
                ...
                #     print('extrapolate', diff)
                    ...
                    # print("COMAND FOR RTDE", pose_command)
                    vel = 0.5
                    acc = 0.5
                ...
                # print("CURRENT POSE:" , state['ActualTCPPose'])
                ...
                try:
                    ...
                    n_cmd = len(commands['cmd'])
                    # print("COMANDS FOR RTDE", self.input_queue.get_all())
                except Empty:
                    n_cmd = 0
                ...
                for i in range(n_cmd):
                    ...
                    elif cmd == Command.UNPAUSE.value:
                        rtde_c = RTDEControlInterface(hostname=robot_ip)
                        # set parameters
                        if self.tcp_offset_pose is not None:
                            rtde_c.setTcp(self.tcp_offset_pose)
