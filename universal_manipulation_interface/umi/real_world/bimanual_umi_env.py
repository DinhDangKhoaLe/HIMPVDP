...
class BimanualUmiEnv:
    def __init__(self, 
        ...
        for rc in robots_config:
            ...
            if rc['robot_type'].startswith('ur10'):   
                ...
                this_robot = RTDEInterpolationController(
                    ...
                    frequency=500 if rc['robot_type'] == 'ur10e' else 125,
                    lookahead_time=0.1,
                    gain=300,
        ...
        for gc in grippers_config:
            this_gripper = GripperController(
                shm_manager=shm_manager,
                serial_name=gc['serial_name'],
                baudrate=gc['baudrate'],
    ...
    def get_obs(self) -> dict:
        ...
        k = math.ceil(
            ...
            * (60 / self.frequency)) + 1 # here 2 is adjustable, typically 1 should be enough
